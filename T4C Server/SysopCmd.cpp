...
//////////////////////////////////////////////////////////////////////////////////////////
{
        ...
        if( regKey.Open( HKEY_LOCAL_MACHINE, T4C_KEY GEN_CFG_KEY ) )
		{
			...
			else
			{
				...
				else
				{
					...
					user->Unlock(); */ // steph d�sactivation
	...
	COMMAND( "REMOVE $'S SHOUTS", GOD_CAN_REMOVE_SHOUTS )
		...
		if( target != NULL ){
            if( SUPER_USER ){
				...
				target->Lock();
				target->boCanShout = FALSE;
				target->lCutExpireDate = 0;//BLBLB dans le cas d'un cut sans dur�e, c'est infini.
				target->self->SendSystemMessage( csTemp, RGB( 255, 0, 0 ) );
				target->Unlock();
	...
	COMMAND( "SUMMON MONSTER $ *$", GOD_CAN_SUMMON_MONSTERS )
		...
		if( atoi(PARAM(1)) > 200 && user->self->boAuthGM == false ){
			...
			int lineSize = floor(sqrt((double)atoi(PARAM(1))));//BLBLBL on arrondi, autrement on risque de g�n�rer des positions invalides.. // steph ajout de (double)
			...
			for (int i=0; i< atoi(PARAM(1)); i++) {
				if (i % lineSize == 0) {
					wlPos.Y++;//on passe � la ligne suivante
					wlPos.X -= lineSize; //et on se remet en d�but de ligne
				} else wlPos.X++;
...
/*			for (int i=0; i>=0 && i< atoi(PARAM(1)); i++) {
    ...
    COMMAND( "SUMMON CIRCLE OF ITEM $ AT $,$,$", GOD_CAN_SUMMON_ITEMS )
		...
		if( dwID != 0 && user->self->boAuthGM == true ){
			...
			for (int a=0;a<180;a+=10){//on parcours les angles de 0 � 180 par incr�ment de 10.
    ...
    COMMAND( "SUMMON CIRCLE OF ITEM $", GOD_CAN_SUMMON_ITEMS )
		...
		if( dwID != 0 && user->self->boAuthGM == true ){
			...
			WorldPos wlPos = user->self->GetWL();//on prend les coordonn�es du lanceur de la commande
			WorldPos wlInitial = wlPos;
...
#define COMPONENT( __cmp, __god,__pos ) }else if( user->GetGodFlags() & __god && stricmp( (LPCTSTR)PARAM( __pos ), __cmp ) == 0  ){
	...
	COMMAND( "LOCKOUT $ FOR $ DAYS", GOD_CAN_LOCKOUT_USER )
		...
		if (nDaysLocked <= 0) nDaysLocked=1; //BLBLBL le d�lai de lock minimum en jours est de 1.
	...
	//////////////////////////////////////////////////////////////////////////////////////////	
	// Ban temporarly user from the game.
	COMMAND( "LOCKOUT $ FOR $ HOURS", /*GOD_CAN_LOCKOUT_USER*/ GOD_CAN_ZAP )//BLBLBL : pour les lock temporaires inf�rieurs � 24H on peut donner cette fonction � ceux pouvant ZAPPER.
		if( user->self->boAuthGM == false )
		{
			_LOG_GAMEOP
				LOG_CRIT_ERRORS,
		...
		if (nHoursLocked > 48) nHoursLocked=48;//BLBLBL limitation en nombre d'heures � 48 heures, ce qui est d�j� beaucoup !
		else if (nHoursLocked <= 0) nHoursLocked=1; //BLBLBL on ne peut pas mettre � 0 le nombre d'heures restantes, c'est 1 minimum.
