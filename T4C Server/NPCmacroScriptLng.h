...
void EXPORT NPCFormatMsg( String &msg, String &parammsg );
...
#define InitTalk ;char lpszDummy[ 16 ];\
    ...
    String msg = (LPCTSTR)valueIN;/* allows single word commands */\
    String parammsg;\
    ...
    if( !( self->GetTarget() == target || self->IsPrivateTalk() ) ){\
        ...
        target->SendPrivateMessage( String( INTL( 7290, "Sorry but I am talking to someone else." ) ), self, npc.dwTextColor );\
        return;\
    }\
...
#define EndTalk            ;} /* Begin */\
                           ;if( !output.IsEmpty() ){\
                                if( self->IsPrivateTalk() ){\
                                    ...
                                    self->Talk( output.c_str(), npc.dwTextColor, target );\
                                }\
                            }\
...
bool EXPORT NPCKeyWord( const String &msg, const char *cmd );
...
#define NUM_PARAM( __num )	NPCatoi( lpcsParams[ __num ].c_str() )

bool EXPORT NPCGetParameters( const char *cmd, String &parammsg, String *lpcsParams );
...
BOOL EXPORT _ORDER ( String Message, LPCTSTR cmd1, LPCTSTR cmd2, LPCTSTR cmd3 = NULL, LPCTSTR cmd4 = NULL, LPCTSTR cmd5 = NULL);
...
void EXPORT SendSellItemListFunc( String &output, LPCTSTR text, TemplateList< _OBJECTITEM > &tlItemList, Unit *target );
#define SendSellItemList( text ) SendSellItemListFunc( output, text, tlItemList, target ); };

/////////////////////////////////////////////////////////////////////////////////////////
// Returns TRUE if skill can be taugh
#define CanTeach(THE_SKILL)		{String dummy;Skills::IsSkillLearnable(THE_SKILL, target, dummy);}
