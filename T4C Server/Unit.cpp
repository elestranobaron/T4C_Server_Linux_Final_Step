...
Unit::Unit( bool boDbgLock )
{
	...
	userSpeed = 1;//Nombre de cases pour chaque ordre de dיplacement
...
WorldPos Unit::MoveUnit(DIR::MOVE where, BOOL boAbsolute, bool boCompressMove, bool boBroadcastMove){	
	...
	if(!ViewFlag(__FLAG_STUN)){
		...
		//4 aout 2009 : apparement חa cause des lags d'aprטs les joueurs... on enlטve א nouveau pour rייssayer.
		//13 janvier : nouvelle tentative !! oula, bah en fait חa fait carrי rouge sur carrי rouge et 
		//apparement des pointes א 6mb d'upload sur la bande passante du servuer ...
		//boSendMove = true;//BLBLBL 28 juillet 2009 : test de toujours envoyer le broadcast, (donc plus de "compression")
						  //en espיrant que cela rיduise les ghosts de monstres qui du coup peuvent ne pas se trouver
						  //א l'endroit oש le client pense qu'ils sont.
						  //en espיrant aussi rיduire les "sauts temporels" en cas de lag ou de speedhack volontaire
			...
			if( UnitType == U_PC ){
...

                {
                    ...
                    if( pPlayer != NULL ){
						...
						//BLBLBL Truc bizare ici, א vיrifier pourquoi on sort du ingame
						//חa a l'air de causer des freezes (le joueur ne peut plut bouger/lancer de sort)
						//mais peut continuer א lire les CC et voir les autres choses bouger א son יcran.
				...
				sending.Destroy();//sending est re-peuplי א l'appel de packet_inview_units plusbas
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    Broadcast::BCast( GetWL(), 24, sending );//BLBLBL 20 => 24 (pour que les paroles locales atteignent tout l'יcran 1024)
}
...
//	_LOG_DEBUG LOG_DEBUG_LVL4, "Setting new exhaust Mental %u,Move %u,Atk %u.", sExhaust.mental,sExhaust.move,sExhaust.attack LOG_//BLBLBL ajoutי un peu d'info
...
(
 ...
 int nBroadcast //BLBL dans le prototype de la fonction, il semble que la valeur soit mise א true par dיfaut !
